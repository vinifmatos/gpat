...
export class GruposFormComponent extends FormComponentBase {
  ...
  constructor(
    ...
    super(api, campos, fb, route, router, api.recursos['grupos'])
  }
  ...
  get_grupos_pai(e: AutoCompleteCompleteEvent) {
    this.api.get<Grupo[]>([this.recurso.rotas.index], { pais: true, query: e.query }).subscribe(
      res => { this.grupos_pai = res.body as Grupo[] },
    )
  }

  protected override before_submit(submited: boolean): void {
    if (this.campos.pai?.id)
